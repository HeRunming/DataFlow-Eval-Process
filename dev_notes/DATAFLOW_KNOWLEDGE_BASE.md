# DataFlow 知识库（Knowledge Base）

> 版本：1.0.10（cc 分支）
> 仓库：https://github.com/HeRunming/DataFlow.git（cc 分支）
> 官方文档：https://opendcai.github.io/DataFlow-Doc/zh/
> 本文件用于在无上下文情况下理解、使用和开发 DataFlow。

---

## 一、项目总体概述

DataFlow 是一个**以数据为中心（Data-Centric AI）**的数据治理系统，面向大语言模型（LLM）的训练数据制备。核心定位：

- **修正、扩增、评估与过滤**低质量数据，生成高质量 LLM 训练数据集
- 支持 PDF 文档、纯文本、爬虫数据、多模态等多种数据来源
- 当前版本：`1.0.10`（`dataflow/version.py`）
- Python 要求：`>= 3.10`
- 包名：`open-dataflow`（PyPI）

### 四大核心组成

| 组件 | 说明 |
|------|------|
| **算子（Operator）** | 原子性数据处理单元，基于规则/DL模型/LLM |
| **流水线（Pipeline）** | 有序连接算子，针对具体数据处理场景 |
| **提示词（Prompt）** | 支持注册和映射的提示词模板 |
| **LLM Serving** | 统一封装本地模型和API推理 |

---

## 二、目录结构

```
DataFlow/
├── dataflow/
│   ├── __init__.py          # 包入口，导出 utils、operators、prompts、logger
│   ├── version.py           # 版本号 1.0.10
│   ├── logger.py            # 日志系统（colorlog，自定义 SUCCESS 级别）
│   ├── cli.py               # CLI 入口（Typer）
│   ├── core/                # 抽象基类层
│   │   ├── operator.py      # OperatorABC, get_operator()
│   │   ├── llm_serving.py   # LLMServingABC
│   │   ├── wrapper.py       # WrapperABC
│   │   ├── prompt.py        # PromptABC, DIYPromptABC, @prompt_restrict 装饰器
│   │   └── __init__.py      # 导出 OPERATOR_CLASSES, LLM_SERVING_CLASSES 等
│   ├── pipeline/
│   │   ├── Pipeline.py      # PipelineABC, BatchedPipelineABC, StreamBatchedPipelineABC
│   │   └── nodes.py         # OperatorNode, KeyNode（DAG 节点）
│   ├── operators/           # 所有算子实现
│   │   ├── general_text/    # 通用文本算子
│   │   ├── text_sft/        # SFT 指令微调算子
│   │   ├── text_pt/         # 预训练文本算子
│   │   ├── reasoning/       # 推理数据算子
│   │   ├── code/            # 代码数据算子
│   │   ├── core_text/       # 核心文本算子
│   │   ├── core_vision/     # 视觉算子
│   │   ├── core_speech/     # 语音算子
│   │   ├── agentic_rag/     # AgenticRAG 算子
│   │   ├── chemistry/       # 化学领域算子
│   │   ├── conversations/   # 对话数据算子
│   │   ├── general_text/    # 通用文本
│   │   ├── knowledge_cleaning/ # 知识库清洗算子
│   │   ├── pdf2vqa/         # PDF→VQA 算子
│   │   ├── text2sql/        # Text→SQL 算子
│   │   └── __init__.py
│   ├── prompts/             # 提示词模板
│   │   ├── general_text.py  # 通用文本 Prompt
│   │   ├── reasoning/       # 推理 Prompt
│   │   └── __init__.py
│   ├── serving/             # LLM/VLM 服务封装
│   │   ├── api_llm_serving_request.py  # 通用 HTTP API（OpenAI 兼容）
│   │   ├── lite_llm_serving.py         # LiteLLM 多提供商
│   │   ├── local_model_llm_serving.py  # vLLM / SGLang 本地模型
│   │   ├── localhost_llm_api_serving.py
│   │   ├── LocalSentenceLLMServing.py  # Embedding 服务
│   │   ├── light_rag_serving.py
│   │   ├── flash_rag_serving.py
│   │   └── __init__.py
│   ├── utils/
│   │   ├── registry.py      # Registry + LazyLoader（核心注册机制）
│   │   ├── storage.py       # DataFlowStorage, FileStorage, LazyFileStorage, DummyStorage
│   │   └── __init__.py      # 导出 OPERATOR_REGISTRY, PROMPT_REGISTRY
│   ├── wrapper/
│   │   ├── auto_op.py       # AutoOP, OPRuntime（Pipeline 编译核心）
│   │   └── batch_wrapper.py # BatchWrapper（批处理包装器）
│   ├── cli_funcs/           # CLI 子命令实现
│   ├── example/             # 示例数据文件（jsonl/json）
│   └── webui/               # WebUI 相关
└── awesome_dataflow.md      # 生态项目列表
```

---

## 三、核心抽象层（`dataflow/core/`）

### 3.1 OperatorABC

所有算子的抽象基类（`dataflow/core/operator.py`）：

```python
class OperatorABC(ABC):
    def __init__(self):
        self.logger = get_logger()
        self.ALLOWED_PROMPTS = tuple([type[DIYPromptABC | PromptABC]])

    @abstractmethod
    def run(self) -> None:
        pass
```

**实现规范**：
1. 继承 `OperatorABC`
2. 用 `@OPERATOR_REGISTRY.register()` 装饰器注册
3. `__init__` 接收超参数（如 `min_words`, `llm_serving` 等）
4. 实现 `run(self, storage: DataFlowStorage, input_key: str, ...)` 方法
5. `run()` 参数命名约定：`input_*` 开头的参数为输入 key，`output_*` 开头的为输出 key，其他参数警告
6. `run()` 必须调用 `storage.read()` 和 `storage.write()`
7. `run()` 返回输出 key 列表（如 `['instruction', 'output']`）
8. 建议实现 `@staticmethod get_desc(lang="zh")` 方法（WebUI 描述用）

**get_operator 工厂函数**：

```python
from dataflow.core import get_operator
op = get_operator("WordNumberFilter", {"min_words": 10, "max_words": 500})
```

### 3.2 LLMServingABC

所有 LLM 服务的抽象基类（`dataflow/core/llm_serving.py`）：

```python
class LLMServingABC(ABC):
    @abstractmethod
    def generate_from_input(self, user_inputs: List[str], system_prompt: str) -> List[str]: ...
    @abstractmethod
    def start_serving(self): ...
    @abstractmethod
    def cleanup(self): ...
    def load_model(self, model_name_or_path: str, **kwargs): ...
```

### 3.3 WrapperABC

包装器的抽象基类，用于对算子进行包装增强（`dataflow/core/wrapper.py`）。

### 3.4 Prompt 系统

```python
# PromptABC: 标准 Prompt 基类
class PromptABC:
    def build_prompt(self): ...

# DIYPromptABC: 用户自定义 Prompt 基类
class DIYPromptABC(PromptABC): ...

# @prompt_restrict 装饰器: 限制算子允许使用的 Prompt 类型
@prompt_restrict(SFTGeneratorSeedPrompt)
class SFTGeneratorSeed(OperatorABC): ...
```

---

## 四、Pipeline 系统（`dataflow/pipeline/`）

### 4.1 三种 Pipeline 基类

| 类名 | 说明 |
|------|------|
| `PipelineABC` | 基础 Pipeline，`forward()` 运行一次 |
| `BatchedPipelineABC` | 支持分批处理，支持 `resume_from_last` |
| `StreamBatchedPipelineABC` | 流式分批，通过 `iter_chunks()` 迭代数据 |

### 4.2 Pipeline 开发方式

```python
from dataflow.pipeline.Pipeline import PipelineABC
from dataflow.utils.storage import FileStorage

class MyPipeline(PipelineABC):
    def __init__(self):
        super().__init__()
        # 在这里实例化算子（成员变量）
        self.op1 = WordNumberFilter(min_words=10, max_words=500)
        self.op2 = SFTGeneratorSeed(llm_serving=my_serving, custom_prompt="...")

    def forward(self):
        # 在编译阶段，self.op1/op2 被包装为 AutoOP
        # AutoOP.run() 不立即执行，而是记录 OPRuntime 供后续执行
        storage = FileStorage(
            first_entry_file_name="input.jsonl",
            cache_path="./cache",
            file_name_prefix="step",
            cache_type="jsonl"
        )
        self.op1.run(storage=storage.step(), input_key="text")
        self.op2.run(storage=storage.step(), input_key="text")

# 使用方式
pipeline = MyPipeline()
pipeline.compile()     # 编译：解析 DAG，验证 key 完整性
pipeline.forward()     # 执行：按顺序运行所有算子
```

### 4.3 Pipeline 编译机制

`compile()` 过程：
1. 将所有 `OperatorABC` 成员变量替换为 `AutoOP` 包装
2. 执行 `forward()` 记录所有 `OPRuntime`（不实际运行算子）
3. 调用 `_build_operator_nodes_graph()` 构建有向无环图（DAG）
4. 验证 key 完整性：每个算子的 `input_*` key 必须在上游算子的 `output_*` key 或初始数据集列中存在
5. Key 验证失败会抛出 `KeyError` 并打印详细信息

`forward()` 运行时：
- 顺序执行每个 `OperatorNode`
- 自动管理 LLM Serving 生命周期（引用计数归零时调用 `cleanup()`）
- 支持 `resume_step` 断点续传

### 4.4 DAG 可视化

```python
pipeline.compile()
pipeline.draw_graph(port=8080, hide_no_changed_keys=True)
# 需要安装: pip install pyvis
# 会启动本地 HTTP 服务，在浏览器中展示交互式 DAG
```

### 4.5 key 命名约定

在 `run()` 参数中：
- `input_*`（如 `input_key`, `input_text`）→ 标记为 DAG 输入依赖
- `output_*`（如 `output_key`, `output_label`）→ 标记为 DAG 输出产物
- 其他参数 → 在 `OperatorNode._get_keys_from_kwargs()` 中会打印警告（非 key 参数，正常）
- `storage` 参数由 Pipeline 自动注入，不算 key

---

## 五、存储系统（`dataflow/utils/storage.py`）

### 5.1 存储类型

| 类名 | 说明 | 适用场景 |
|------|------|---------|
| `FileStorage` | 每次 read/write 立即落盘 | 通用，简单可靠 |
| `LazyFileStorage` | 内存中操作，进程退出时落盘（原子写） | 高性能，防止部分写 |
| `DummyStorage` | 纯内存，不落盘（可选配 cache_type 落盘） | BatchWrapper 内部使用 |

### 5.2 FileStorage 使用

```python
from dataflow.utils.storage import FileStorage

storage = FileStorage(
    first_entry_file_name="./data/input.jsonl",   # 初始数据文件
    cache_path="./cache",                          # 中间结果目录
    file_name_prefix="dataflow_cache_step",        # 缓存文件前缀
    cache_type="jsonl"                             # 缓存格式: json/jsonl/csv/parquet/pickle
)

# 算子内部使用：
df = storage.read("dataframe")   # 读为 DataFrame
storage.write(filtered_df)        # 写出结果

# Pipeline 中，每个算子使用 storage.step() 获取步进副本
```

### 5.3 远程数据源支持

`first_entry_file_name` 支持三种格式：
- 本地文件路径：`./data/input.jsonl`
- HuggingFace：`hf:openai/gsm8k:main:train`（`hf:{dataset}[:config][:split]`）
- ModelScope：`ms:modelscope/gsm8k:train`（`ms:{dataset}[:split]`）

### 5.4 LazyFileStorage 特性

- 数据在内存中缓冲，进程正常退出/SIGINT/SIGTERM 时自动 flush
- 原子落盘（`os.replace`），线程安全（`RLock`）
- `flush_all_steps=False`（默认）：只保留最新步骤；`True`：保留所有步骤
- 适合推理时间长、不想每步都写磁盘的场景

---

## 六、LLM Serving 系统（`dataflow/serving/`）

### 6.1 可用 Serving 实现

| 类名 | 说明 |
|------|------|
| `APILLMServing_request` | 通用 HTTP API，支持 OpenAI 兼容接口，多线程并发 |
| `LiteLLMServing` | 基于 LiteLLM，支持 OpenAI/Anthropic/Azure/Bedrock 等众多提供商 |
| `LocalModelLLMServing_vllm` | 本地 vLLM 推理服务 |
| `LocalModelLLMServing_sglang` | 本地 SGLang 推理服务 |
| `LocalHostLLMAPIServing_vllm` | 本地已启动的 vLLM API |
| `APIVLMServing_openai` | 视觉语言模型 API（OpenAI 格式） |
| `LocalVLMServing_vllm` | 本地 VLM（vLLM）服务 |
| `LocalModelLALMServing_vllm` | 本地 LALM（语音）vLLM 服务 |
| `LocalEmbeddingServing` | 本地 Sentence Embedding 服务 |
| `LightRAGServing` | LightRAG 服务 |
| `PerspectiveAPIServing` | Google Perspective API |

### 6.2 APILLMServing_request 使用

```python
import os
os.environ["DF_API_KEY"] = "your-api-key"  # API key 必须通过环境变量注入

from dataflow.serving import APILLMServing_request

serving = APILLMServing_request(
    api_url="https://api.openai.com/v1/chat/completions",
    key_name_of_api_key="DF_API_KEY",   # 环境变量名
    model_name="gpt-4o",
    temperature=0.0,
    max_workers=10,       # 并发线程数
    max_retries=5,
    connect_timeout=10.0,
    read_timeout=120.0,
)

responses = serving.generate_from_input(
    user_inputs=["问题1", "问题2"],
    system_prompt="You are a helpful assistant",
    json_schema=None   # 可选，结构化输出
)
```

**注意**：API key **必须通过环境变量**传入，不能直接在代码中写。

### 6.3 LiteLLMServing 使用

```python
from dataflow.serving import LiteLLMServing
# 需要安装: pip install open-dataflow[litellm] 或 pip install litellm

serving = LiteLLMServing(
    model_name="gpt-4o",          # 或 claude-3-sonnet 等
    key_name_of_api_key="DF_API_KEY",
    temperature=0.7,
    max_tokens=1024,
    max_workers=10,
    validate_on_init=True,        # 初始化时发送测试请求验证
)
```

### 6.4 Serving 生命周期

Pipeline 会自动管理 Serving 的生命周期：
- 使用引用计数（`llm_serving_counter`）
- 当一个 Serving 的所有算子执行完毕后，自动调用 `cleanup()`
- 切换到不同 Serving 时，先清理上一个

---

## 七、算子注册系统（`dataflow/utils/registry.py`）

### 7.1 Registry 机制

```python
from dataflow.utils.registry import OPERATOR_REGISTRY, PROMPT_REGISTRY

# 注册算子
@OPERATOR_REGISTRY.register()
class MyFilter(OperatorABC): ...

# 注册 Prompt
@PROMPT_REGISTRY.register()
class MyPrompt(PromptABC): ...
```

### 7.2 LazyLoader 机制

每个算子子模块（如 `general_text`, `text_sft` 等）的 `__init__.py` 采用 `LazyLoader` 模式：

```python
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    # 仅用于静态分析（IDE 提示）
    from .filter.word_number_filter import WordNumberFilter
else:
    # 运行时：按需懒加载
    import sys
    from dataflow.utils.registry import LazyLoader, generate_import_structure_from_type_checking
    cur_path = "dataflow/operators/general_text/"
    _import_structure = generate_import_structure_from_type_checking(__file__, cur_path)
    sys.modules[__name__] = LazyLoader(__name__, cur_path, _import_structure)
```

**优点**：启动时无需加载所有算子，用到才加载，节省内存和启动时间。
**注意**：新增算子必须在对应模块的 `__init__.py` 的 `TYPE_CHECKING` 块中声明导入，否则 `LazyLoader` 无法发现。

---

## 八、算子分类与功能概述

### 8.1 `general_text`（通用文本）

**Filter**：
- 规则型：`ColonEndFilter`, `SentenceNumberFilter`, `ContentNullFilter`, `SymbolWordRatioFilter`, `AlphaWordsFilter`, `WordNumberFilter`, `CharNumberFilter`, `MeanWordLengthFilter`, `StopWordFilter`, `NoPuncFilter`, `SpecialCharacterFilter`, `WatermarkFilter`, `CurlyBracketFilter`, `CapitalWordsFilter`, `LoremIpsumFilter`, `UniqueWordsFilter`, `LineStartWithBulletpointFilter`, `LineWithJavascriptFilter`, `LineEndWithEllipsisFilter`, `HtmlEntityFilter`, `IDCardFilter`
- 语言类：`LanguageFilter`, `LLMLanguageFilter`
- 去重类：`HashDeduplicateFilter`, `MinHashDeduplicateFilter`, `NgramHashDeduplicateFilter`, `SimHashDeduplicateFilter`, `SemDeduplicateFilter`
- 其他：`LangkitFilter`, `LexicalDiversityFilter`, `NgramFilter`, `PresidioFilter`, `BlocklistFilter`, `PerspectiveFilter`

**Refine**：HTML实体、URL移除、小写转换、NER、PII匿名化、引用移除、缩写扩展、emoji移除、多余空格移除、图片引用移除、数字/标点移除、停用词移除、拼写校正、词干化/词形还原、文本规范化等

**Eval**：Ngram、词汇多样性、Langkit、Presidio、BERT、BLEU、CIDEr、Perspective、Task2Vec、Vendi

### 8.2 `text_sft`（SFT 指令微调）

**Eval**：`AlpagasusSampleEvaluator`, `DeitaQualitySampleEvaluator`, `DeitaComplexitySampleEvaluator`, `InstagSampleEvaluator`, `RMSampleEvaluator`, `SuperfilteringSampleEvaluator`, `TreeinstructSampleEvaluator`

**Filter**：对应上述各评估算子的过滤版本

**Generate**：
- `CondorGenerator`：Condor 数据生成
- `SFTGeneratorSeed`：从种子文档生成 SFT 格式 instruction-output 对

**Refine**：`CondorRefiner`

### 8.3 `reasoning`（推理数据）

**Generate**：`ReasoningAnswerGenerator`, `ReasoningQuestionGenerator`, `ReasoningAnswerExtractionQwenMathEvalGenerator`, `ReasoningPseudoAnswerGenerator`, `ReasoningPretrainFormatConvertGenerator`, `ReasoningQuestionFusionGenerator`

**Eval**：`ReasoningCategoryDatasetEvaluator`, `ReasoningDifficultyDatasetEvaluator`, `ReasoningTokenDatasetEvaluator`, 以及 sample 级别的难度/分类/可解性评估

**Filter**：`ReasoningAnswerFormatterFilter`, `ReasoningAnswerGroundTruthFilter`, `ReasoningAnswerNgramFilter`, `ReasoningAnswerPipelineRootFilter`, `ReasoningAnswerTokenLengthFilter`, `ReasoningQuestionFilter`, `ReasoningAnswerModelJudgeFilter`

### 8.4 `code`（代码数据）

包含 `eval/`, `filter/`, `generate/` 三类子目录，具体算子参见 `__init__.py`。

### 8.5 其他算子模块

- `agentic_rag`：AgenticRAG 相关算子
- `chemistry`：化学领域数据
- `conversations`：对话数据
- `knowledge_cleaning`：知识库清洗
- `pdf2vqa`：PDF → VQA 数据生成
- `text2sql`：Text2SQL 数据
- `core_text`：核心文本算子（含 generate/filter/eval）
- `core_vision`：视觉算子
- `core_speech`：语音算子
- `text_pt`：预训练文本算子
- `general_text`：通用文本算子

---

## 九、Prompt 系统（`dataflow/prompts/`）

### 9.1 注册与使用

```python
from dataflow.utils.registry import PROMPT_REGISTRY
from dataflow.core.prompt import PromptABC

@PROMPT_REGISTRY.register()
class MyPrompt(PromptABC):
    def __init__(self, custom_param="default"):
        self.custom_param = custom_param

    def build_prompt(self, content: str) -> str:
        return f"System: {self.custom_param}\nUser: {content}"
```

### 9.2 `@prompt_restrict` 装饰器

用于限制算子只能接受特定 Prompt 类型：

```python
from dataflow.core.prompt import prompt_restrict

@prompt_restrict(MyPrompt)
class MyOperator(OperatorABC):
    def __init__(self, llm_serving, prompt_template=None):
        # prompt_template 只能是 MyPrompt 实例或 DIYPromptABC 子类
        self.prompt_template = prompt_template
```

### 9.3 DIYPromptABC

用户自定义 Prompt，可绕过 `@prompt_restrict` 的白名单限制：

```python
from dataflow.core.prompt import DIYPromptABC

class MyCustomPrompt(DIYPromptABC):
    def build_prompt(self, content: str) -> str:
        return f"Custom: {content}"
```

---

## 十、日志系统（`dataflow/logger.py`）

```python
from dataflow import get_logger
logger = get_logger()

# 日志级别
logger.debug("debug message")
logger.info("info message")
logger.success("success message")    # 自定义 SUCCESS 级别（绿色）
logger.warning("warning message")
logger.error("error message")
logger.critical("critical message")

# 控制日志级别（通过环境变量）
# export DF_LOGGING_LEVEL=DEBUG
```

**特性**：
- 日志器名称：`DataFlow`
- `< ERROR` → stdout；`>= ERROR` → stderr
- colorlog 着色，格式类似 loguru
- 避免重复 handler

---

## 十一、CLI 系统（`dataflow/cli.py`）

使用 Typer 构建，入口：`dataflow`

```bash
# 查看版本
dataflow --version

# 查看环境信息
dataflow env

# 初始化
dataflow init                          # base 初始化
dataflow init repo                     # 初始化仓库脚手架
dataflow init operator                 # 初始化算子（TODO）
dataflow init pipeline                 # 初始化 pipeline（TODO）

# 模型评估
dataflow eval init
dataflow eval api
dataflow eval local

# PDF → 模型训练
dataflow pdf2model init --qa kbc       # 或 --qa vqa
dataflow pdf2model train

# 文本 → 模型训练
dataflow text2model init
dataflow text2model train

# WebUI
dataflow webui --host 0.0.0.0 --port 8000

# 聊天
dataflow chat
dataflow chat --model /path/to/model
```

---

## 十二、BatchWrapper（`dataflow/wrapper/batch_wrapper.py`）

用于将任意算子包装为批处理模式：

```python
from dataflow.wrapper.batch_wrapper import BatchWrapper

wrapped_op = BatchWrapper(my_operator, batch_size=32, batch_cache=False)
wrapped_op.run(storage=storage, input_key="text", output_key="label")
```

- 内部将全量数据切分为多个批次
- 每批次使用 `DummyStorage` 隔离运行
- 结果合并回原始 DataFrame

---

## 十三、新增算子完整流程

### 步骤 1：创建算子文件

在合适的目录（如 `dataflow/operators/general_text/filter/`）创建 Python 文件：

```python
# dataflow/operators/general_text/filter/my_filter.py
from tqdm import tqdm
from dataflow import get_logger
from dataflow.core import OperatorABC
from dataflow.utils.storage import DataFlowStorage
from dataflow.utils.registry import OPERATOR_REGISTRY

@OPERATOR_REGISTRY.register()
class MyFilter(OperatorABC):
    def __init__(self, threshold: float = 0.5):
        super().__init__()   # 初始化 logger
        self.threshold = threshold
        self.logger.info(f"Initializing {self.__class__.__name__}...")

    @staticmethod
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            return "算子描述（中文）..."
        else:
            return "Operator description (English)..."

    def run(self, storage: DataFlowStorage, input_key: str, output_key: str = "my_filter_label"):
        self.logger.info(f"Running {self.__class__.__name__}...")
        dataframe = storage.read("dataframe")

        # 处理逻辑
        results = []
        for text in tqdm(dataframe[input_key], desc=f"Running {self.__class__.__name__}"):
            results.append(len(text) > self.threshold)

        dataframe[output_key] = results
        filtered_df = dataframe[dataframe[output_key]]
        storage.write(filtered_df)
        self.logger.info(f"Filter completed. Kept {len(filtered_df)} rows.")
        return [output_key]
```

### 步骤 2：在 `__init__.py` 中注册

在 `dataflow/operators/general_text/__init__.py` 的 `TYPE_CHECKING` 块中添加：

```python
if TYPE_CHECKING:
    # ...已有导入...
    from .filter.my_filter import MyFilter
```

### 步骤 3：测试

```python
from dataflow.operators.general_text import MyFilter
from dataflow.utils.storage import FileStorage

op = MyFilter(threshold=10)
storage = FileStorage("data.jsonl")
storage.step()
op.run(storage, input_key="text")
```

---

## 十四、新增 Pipeline 完整流程

```python
from dataflow.pipeline.Pipeline import PipelineABC
from dataflow.utils.storage import FileStorage, LazyFileStorage
from dataflow.operators.general_text import WordNumberFilter
from dataflow.operators.text_sft import SFTGeneratorSeed
from dataflow.serving import APILLMServing_request
import os

os.environ["DF_API_KEY"] = "sk-xxx"

class MySFTPipeline(PipelineABC):
    def __init__(self):
        super().__init__()
        self.serving = APILLMServing_request(
            api_url="https://api.openai.com/v1/chat/completions",
            key_name_of_api_key="DF_API_KEY",
            model_name="gpt-4o",
        )
        self.word_filter = WordNumberFilter(min_words=20, max_words=100000)
        self.sft_gen = SFTGeneratorSeed(
            llm_serving=self.serving,
            custom_prompt="Generate high-quality QA pairs."
        )

    def forward(self):
        storage = FileStorage(
            first_entry_file_name="input.jsonl",
            cache_path="./cache",
            file_name_prefix="sft_pipeline",
            cache_type="jsonl"
        )
        self.word_filter.run(storage=storage.step(), input_key="text", output_key="word_count")
        self.sft_gen.run(storage=storage.step(), input_key="text")

if __name__ == "__main__":
    pipeline = MySFTPipeline()
    pipeline.compile()         # 编译并验证 key
    pipeline.forward()         # 运行
    # pipeline.draw_graph()    # 可视化 DAG
```

---

## 十五、BatchedPipeline 用法

```python
from dataflow.pipeline.Pipeline import BatchedPipelineABC

class MyBatchedPipeline(BatchedPipelineABC):
    def __init__(self): ...
    def forward(self): ...

pipeline = MyBatchedPipeline()
pipeline.compile()
pipeline.forward(
    batch_size=100,           # 每批 100 条
    resume_from_last=True     # 从上次中断处继续
)
```

断点续传机制：在 cache 目录下写 `{prefix}_last_success_step.txt`，格式为 `step,batch`。

---

## 十六、安装说明

### 用户安装（PyPI）

```bash
pip install open-dataflow
# 或使用 uv:
uv pip install open-dataflow

# GPU 后端（vLLM）:
pip install open-dataflow[vllm]
# SGLang:
pip install open-dataflow[sglang]
# LiteLLM:
pip install open-dataflow[litellm]
```

### 开发者安装（可编辑模式）

```bash
git clone -b cc https://github.com/HeRunming/DataFlow.git
cd DataFlow
pip install -e .     # 或 uv pip install -e .
```

---

## 十七、示例数据位置

`dataflow/example/` 目录下包含各 Pipeline 的示例数据：

| 目录 | 内容 |
|------|------|
| `GeneralTextPipeline/` | `filtering.json`, `sft_input.jsonl`, `pt_input.jsonl`, `translation.jsonl` 等 |
| `ReasoningPipeline/` | 推理数据示例 |
| `AgenticRAGPipeline/` | RAG 示例数据 |
| `KBCleaningPipeline/` | 知识库清洗示例 |
| `PDF2VQAPipeline/` | PDF→VQA 示例 |
| `CodePipeline/` | 代码数据示例 |
| `Text2SQLPipeline/` | SQL 数据示例 |
| `chemistry/` | 化学数据示例 |
| `VQA/` | VQA 示例 |
| `SpeechTranscription/` | 语音转录示例 |
| `FuncCallPipeline/` | 函数调用示例 |
| `DataflowAgent/` | Agent 示例 |

---

## 十八、常见设计模式总结

### 模式 1：纯规则过滤

```python
@OPERATOR_REGISTRY.register()
class MyRuleFilter(OperatorABC):
    def __init__(self, threshold=0.5):
        super().__init__()
        self.threshold = threshold

    def run(self, storage, input_key, output_key="label"):
        df = storage.read("dataframe")
        df[output_key] = df[input_key].apply(lambda x: ...)
        storage.write(df[df[output_key]])
        return [output_key]
```

### 模式 2：LLM 驱动算子

```python
@OPERATOR_REGISTRY.register()
class MyLLMOperator(OperatorABC):
    def __init__(self, llm_serving: LLMServingABC):
        super().__init__()
        self.llm_serving = llm_serving  # Pipeline 会检测此字段管理生命周期

    def run(self, storage, input_key, output_key="result"):
        df = storage.read("dataframe")
        prompts = [f"处理：{row[input_key]}" for _, row in df.iterrows()]
        results = self.llm_serving.generate_from_input(prompts)
        df[output_key] = results
        storage.write(df)
        return [output_key]
```

### 模式 3：多输出 key

```python
def run(self, storage, input_key, output_instruction="instruction", output_answer="output"):
    ...
    df["instruction"] = ...
    df["output"] = ...
    storage.write(df)
    return ["instruction", "output"]
```

---

## 十九、项目来源与生态

- **主仓库**：https://github.com/OpenDCAI/DataFlow
- **本开发仓库**（cc 分支）：https://github.com/HeRunming/DataFlow.git
- **文档**：https://opendcai.github.io/DataFlow-Doc/zh/
- **WebUI 仓库**：https://github.com/OpenDCAI/DataFlow-WebUI

---

*最后更新：2026-03-29*
